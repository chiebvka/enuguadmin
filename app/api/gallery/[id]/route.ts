...
//         return NextResponse.json({ error: "Gallery ID is missing" }, { status: 400 });
//     }
//     console.log(`API DELETE /api/gallery/${galleryId}: Attempting delete...`);

//     try {
//         // Step 1: Delete related tags (gallerypost_tags)
//         const { error: deleteTagsError } = await supabase
//             .from('gallerypost_tags')
//             .delete()
//             .eq('gallery_id', galleryId);
//         if (deleteTagsError) console.error(`Error deleting gallery tags for ${galleryId}:`, deleteTagsError); // Log but continue

//         // Step 2: Delete related images (galleryimages)
//         // **Important**: This deletes the DB records. You might need separate logic to delete files from R2 storage.
//         const { error: deleteImagesError } = await supabase
//             .from('galleryimages')
//             .delete()
//             .eq('gallery_id', galleryId);
//         if (deleteImagesError) console.error(`Error deleting gallery images for ${galleryId}:`, deleteImagesError); // Log but continue

//         // Step 3: Delete the gallery post itself
//         const { error: deletePostError } = await supabase
//             .from('galleryposts')
//             .delete()
//             .eq('id', galleryId);
//             // Add .eq('author_id', user.id) if only authors can delete their own

//         if (deletePostError) {
//             console.error(`Supabase gallery post delete error for ${galleryId}:`, deletePostError);
//             return NextResponse.json({ error: `Failed to delete gallery post: ${deletePostError.message}` }, { status: 500 });
//         }

//         // Check if deletion was successful (Supabase delete doesn't return data reliably, error is main indicator)

//         console.log(`API DELETE /api/gallery/${galleryId}: Successfully deleted gallery post.`);
//         return NextResponse.json({ message: "Gallery post deleted successfully" }, { status: 200 });

//     } catch (err: any) {
//         console.error(`Gallery deletion failed for ID ${galleryId}:`, err);
//         return NextResponse.json({ error: "Internal server error: " + (err.message || "") }, { status: 500 });
//     }
// }
